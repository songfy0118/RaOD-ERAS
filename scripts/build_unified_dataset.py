...
def binary_gt(gt_path: Path, positive_label: int, ignore_label: int) -> tuple[Image.Image, dict[str, int]]:
...
def main() -> None:
    ...
    for source in SOURCES:
        ...
        for image_path in sorted(cfg.image_dir.glob(cfg.image_glob)):
            ...
            gt_img, stats = binary_gt(gt_path, source.positive_label, cfg.ignore_label)
            ...
            rows.append(
                {
                    ...
                    "gt_binary_path": gt_target.relative_to(out_root).as_posix(),
                    **stats,
                }
            )
